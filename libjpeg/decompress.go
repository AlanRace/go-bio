...
func NewJPEGFromHeader(r io.Reader) (*JPEG, error) {
	...
	j.dinfo = newDecompress(r)
	if j.dinfo == nil {
		return nil, errors.New("allocation failed")
	}

	err := readHeader(j.dinfo, false)
	if err != nil {
		return nil, err
	}
...
func readHeader(dinfo *C.struct_jpeg_decompress_struct, requireImage bool) error {
	if requireImage {
		if C.read_header(dinfo, C.TRUE) != 0 {
			return errors.New(jpegErrorMessage(unsafe.Pointer(dinfo)))
		}
	} else {
		if C.read_header(dinfo, C.FALSE) != 0 {
			return errors.New(jpegErrorMessage(unsafe.Pointer(dinfo)))
		}
	}
